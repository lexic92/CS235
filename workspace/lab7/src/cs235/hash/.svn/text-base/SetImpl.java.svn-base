...
public class SetImpl implements java.util.Set {
	...
	private double hash_table_array_size;
	private List[] array;
	private double loadFactor; // "N/T"  N: numElements. T: hash_table_array_size.
	 ...
	 */
	private void reHash(){
		double old_table_size = hash_table_array_size; //Save old table size;
		...
		array = new LinkedList[(int)hash_table_array_size];
	...
	private int myHashCode(Object e){
		return Math.abs(e.hashCode() % (int)hash_table_array_size);
	}
	 ...
	 * COMPLETE (MAY BE MESSED UP THOUGH)
	 * 
	 * Returns the equivalent object (or Pair, when finding
	 * a key) if this set contains the object. Else, returns null.
	 * 
	 * 
	 * 
	 * @param o the tested object (to return an equivalent object of
	 * this set)
	 * @return the equivalent object if it exists, or null if it doesn't.
	 */
	public Object find(Object o) {
		int index = myHashCode(o);
		for(Object e : array[index]){
			if(e.equals(o)){
				return e; //If e's key is the same as the object...
			}
		}
		return null;
	}
	...
	public void clear() {
		...
		//Don't forget! Initialize the one and only element 
		//of this array with an empty linked list!!!
		array[0] = new LinkedList<Object>();
	...
	public boolean remove(Object o) {
		...
		if(o == null){
			return false; //Assume we can't store nulls in this map.
		}
		int index = myHashCode(o);
		for(Object e : array[index]){
			if(e.equals(o)){
