...
public class MapImpl implements java.util.Map{
	...
	public Object put(Object key, Object value) {
		...
		Object previousPairWithThatKey = ((SetImpl) s).find(putThis);
		if(previousPairWithThatKey == null){
			//It didn't used to exist, and is adding a new pair to the map.
			size++;
			s.add(putThis);
			return null;
		}
		else{
			s.remove(previousPairWithThatKey); //size--
			s.add(putThis);//size++  (they cancel eachother out)
			return ((Pair) previousPairWithThatKey).getvalue();
		}
		
		/* 
		//Object previousPair = ((SetImpl) s).find(putThis);
		
		//Find out if it should overwrite.
		if(s.contains(putThis)){
			//If it contained it before, then overwrite.
			overwriteMe = ((SetImpl)s).find(putThis);
			s.remove(overwriteMe);
		}
		//if it didn't contain it before, then just add it.
		s.add(putThis); //Only works if it is the same key.
		
		//If the previous thing was not null
		if(overwriteMe != null){
			if(((Pair)overwriteMe).equals(putThis)){
				return ((Pair) overwriteMe).value;
			}
			else{
				size++;
				return ((Pair) overwriteMe).value;
